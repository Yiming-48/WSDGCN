...
def get_magnetic_signed_Laplacian(edge_index: torch.LongTensor, edge_weight: Optional[torch.Tensor] = None,
    ...
    edge_index = torch.stack([row, col], dim=0)
     ...
     #=== 修改点1：符号化相位参数 ===
    sgn_weight = torch.sign(edge_weight)  # 符号化核心操作,得到sgn(A)
    diff_sgn_attr = torch.cat([sgn_weight, -sgn_weight], dim=0)  #获取sgn(A)-sgn(A^T)
    diff_abs_attr = torch.sign(torch.cat([torch.abs(edge_weight), -torch.abs(edge_weight)], dim=0)) #获取sgn(|A|-|A^T|)
    sgn_weight_squared = sgn_weight ** 2 # 对 sgn(A) 中的每个元素各自平方，得到 [sgn(A)]^2
    theta_attr = torch.cat([sgn_weight_squared, -sgn_weight_squared], dim=0)
    ...
    edge_attr = torch.stack([sym_attr, theta_attr, sym_abs_attr, diff_sgn_attr, diff_abs_attr], dim=1)
    ...
    edge_weight_sym = edge_weight_sym/2
     #通过函数coalesce()完成sgn(A)-sgn(A^T)后得到新的edge_attr[:, 3]，0代表无向号，±1代表单向，±2代表反向
    mask = (edge_attr[:, 3]**2) * (edge_attr[:, 3]**2 - 1)/12  #在出现符号相反方向相反的连边时取值为1，否则为0
    new_term_values =  mask*(1j*edge_attr[:, 4]-1)
    ...
    edge_weight_q = torch.exp(1j * 2* np.pi * q * edge_attr[:, 1]) + new_term_values
